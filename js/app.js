...
sign_up.addEventListener("click", () => {
  ...
  let department = document.querySelector(".selected-department-name").innerHTML;

  let level = document.querySelector(".selected-level-name").innerHTML;

  let email = ""

  if(name.replace(" ","").length ==0){
    showError("Please Enter your Name. It's necessary","Dismiss");
    return;
  }
  ...
  hash = CryptoJS.SHA3(password, { outputLength: 256 }).toString(CryptoJS.enc.Hex);

  //send request to authentification api

    //create data block to send
    const params = {
      matricule: matricule,
      name: name,
...
sign_in.addEventListener("click", () => {
  //Retrieve client's informations
  let matricule = document.getElementById("loginMatricule").value;
  matricule = matricule.toUpperCase();
  if(!matriculePattern.test(matricule)){
...
/*************** ***************
let sign_up_text="Lorem ipsum dolor sit amet consectetur adipisicing elit. Nostrum laboriosam ad deleniti."
let sign_in_text ="Lorem ipsum, dolor sit amet consectetur adipisicing elit. Debitis,ex ratione. Aliquid!"

let signupText= document.querySelector("#sign-up-text");
let signinText= document.querySelector("#sign-in-text");

let l1 = sign_up_text.length;
let l2 = sign_in_text.length;
...
let timer = setInterval(()=>{
  ...
  signupText.innerHTML += sign_up_text[i1] ;
  i1+=1;
}, 100);
/**************** **************/
/*
